#!/usr/bin/env ruby
# chapter 9
# https://pine.fm/LearnToProgram/chap_09.html
a = Array.new  + [12345]  # Array  addition.
b = String.new + 'hello'  # String addition.
c = Time.new

puts 'a = '+a.to_s
puts 'b = '+b.to_s
puts 'c = '+c.to_s

today = Time.mktime(1999, 12, 31, 23, 59, 59)
puts today.to_s
# checkoutperiod = today.mktime(7, 00, 00, 00)
# puts checkoutperiod
week = 60 * 60 * 24 * 7
day = 60 * 60 * 24
hour = 60 * 60
puts week
puts day
puts hour

time2 = today + 1
puts "time2", time2
time3 = today + week
puts "time3", time3
time4 = today + day
puts "time4", time4
puts time3 - time4
puts 8 * hour


colorArray = []
colorHash = {}

colorArray[0] = 'red'
colorArray[1] = 'green'
colorArray[2] = "blue"
colorHash['strings'] = 'red'
colorHash['numbers'] = 'green'
colorHash['frettes'] = 'blue'

colorArray.each do |color|
	puts color
end
colorHash.each do |key, data|
	puts key + ': ' + data
end


class Integer
  def to_eng
    if self == 5
      english = 'five'
    else
      english = 'fifty-eight'
    end

    english
  end
end

puts 6.to_eng
puts 58.to_eng


class Die
  def roll
    1 + rand(6)
  end
end

dice = [Die.new, Die.new]

dice.each do |die|
  puts die.roll
end
puts "====="

class Die

# When an object is created, its initialize method (if it has one defined) is always called.
  def initialize
    roll
  end

  def roll
    @numberShowing = 1 + rand(6)
  end

  def showing
    @numberShowing
  end

end

die = Die.new
die.roll
puts die.showing
die.roll
puts die.showing
puts "====="
puts Die.new.showing
puts Die.new.showing


class Dragon

  def initialize name
    @name = name
    @asleep = false
    @stuffInBelly     = 10  # He's full.
    @stuffInIntestine =  0  # He doesn't need to go.

    puts @name + ' is born.'
  end

  def feed
    puts 'You feed ' + @name + '.'
    @stuffInBelly = 10
    passageOfTime
  end

  def walk
    puts 'You walk ' + @name + '.'
    @stuffInIntestine = 0
    passageOfTime
  end

  def putToBed
    puts 'You put ' + @name + ' to bed.'
    @asleep = true
    3.times do
      if @asleep
        passageOfTime
      end
      if @asleep
        puts @name + ' snores, filling the room with smoke.'
      end
    end
    if @asleep
      @asleep = false
      puts @name + ' wakes up slowly.'
    end
  end

  def toss
    puts 'You toss ' + @name + ' up into the air.'
    puts 'He giggles, which singes your eyebrows.'
    passageOfTime
  end

  def rock
    puts 'You rock ' + @name + ' gently.'
    @asleep = true
    puts 'He briefly dozes off...'
    passageOfTime
    if @asleep
      @asleep = false
      puts '...but wakes when you stop.'
    end
  end

  private

  # "private" means that the methods defined here are
  # methods internal to the object.  (You can feed
  # your dragon, but you can't ask him if he's hungry.)
  # actually copied this part because I dont get it. :(

  def hungry?
    # call method with ? returns boolean
    @stuffInBelly <= 2
  end

  def poopy?
    @stuffInIntestine >= 8
  end

  def passageOfTime
    if @stuffInBelly > 0
      # Move food from belly to intestine.
      @stuffInBelly     = @stuffInBelly     - 1
      @stuffInIntestine = @stuffInIntestine + 1
    else  # starving!
      if @asleep
        @asleep = false
        puts 'He wakes up suddenly!'
      end
      puts @name + ' is starving!  In desperation, he ate YOU!'
      exit  # This quits the program.
    end

    if @stuffInIntestine >= 10
      @stuffInIntestine = 0
      puts 'Whoops!  ' + @name + ' had an accident...'
    end

    if hungry?
      if @asleep
        @asleep = false
        puts 'He wakes up suddenly!'
      end
      puts @name + '\'s stomach grumbles...'
    end

    if poopy?
      if @asleep
        @asleep = false
        puts 'He wakes up suddenly!'
      end
      puts @name + ' does the potty dance...'
    end
  end

end

pet = Dragon.new 'warpigbaby'
pet.feed
pet.toss
pet.walk
pet.putToBed
pet.rock
pet.toss
pet.feed
pet.walk
pet.putToBed